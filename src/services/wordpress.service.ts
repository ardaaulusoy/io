...
export class WordpressService {
  ...
  getRecentPosts(categoryId:number, page:number = 1){
    ...
    let category_url = categoryId? ("&deal_categories=" + categoryId): "";
  ...
  getPostCategories(post){
    ...
    post.deal_categories.forEach(category => {
  ...
  getCategory(category){
    return this.http.get(Config.WORDPRESS_REST_API_URL + "deal_categories/" + category)
    .map(res => res.json());
  }
